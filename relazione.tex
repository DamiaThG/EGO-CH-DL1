...
\begin{titlepage}
\centering
{\bfseries \Large UNIVERSITÀ DEGLI STUDI DI CATANIA \par}
\vspace{0.3cm}
{\Large Corso di Laurea Magistrale in Informatica \par}
\vspace{0.2cm}
{\large \textbf{Progetto di Deep Learning 2} \par}
\vspace{1.5cm}
\rule{\linewidth}{0.6pt} \\[0.4cm]
{\huge \bfseries Progetto EGO-CH --- Deep Learning \\[0.3cm] \Large Relazione Finale \par}
...
\emph{Autori:}\\[0.2cm]
\begin{tabular}{@{} l l @{}}
\textbf{Damiano Messina} & {\small (Task 1: Room Localization)} \\[0.15cm]
\textbf{Claudio Nuncibello} & {\small (Task 2: POI Recognition)} \\[0.15cm]
\textbf{Emanuele Barbagallo} & {\small (Task 3: Object Retrieval)}
\end{tabular}
\end{flushleft}
\end{minipage}%
\hfill
...
\section{Organizzazione del Lavoro e Contributi degli Autori}

Durante lo sviluppo del progetto abbiamo lavorato in costante sinergia, confrontandoci regolarmente e collaborando su ogni fase del lavoro, dalla scelta delle architetture alla stesura del codice e della relazione.

Allo stesso tempo, per organizzare al meglio le attività e approfondire ogni aspetto del benchmark, ciascuno di noi si è focalizzato con maggiore attenzione sulla supervisione e sullo sviluppo di un task specifico:
\begin{itemize}
    \item \textbf{Damiano Messina}: si è occupato in prima persona del \textbf{Task 1 (Room-based Localization)};
    \item \textbf{Claudio Nuncibello}: ha curato direttamente il \textbf{Task 2 (Point of Interest Recognition)};
    \item \textbf{Emanuele Barbagallo}: si è concentrato sullo sviluppo del \textbf{Task 3 (Object Retrieval)}.
\end{itemize}
...
\section{Estrazione delle features: DINOv2}
...
\subsection{Approccio End-to-End (V3)}
...
Mamba V3 & DINOv2 + MLP + MS-TCN (End-to-End) & \textbf{0.8718} & 0.5815 \\
\bottomrule
\end{tabular}%
}
\end{table}
