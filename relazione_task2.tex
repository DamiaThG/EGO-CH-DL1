...
% ================================================================
% CAPITOLO 3 — TASK 2: POINT OF INTEREST RECOGNITION (dettagliato)
% ================================================================
...
La nostra proposta consiste nella sostituzione del backbone YOLOv3 con l'architettura \textbf{YOLOv8}~\cite{yolov8} (Ultralytics, 2023), che rappresenta un salto generazionale nell'evoluzione della famiglia YOLO. Le principali innovazioni architetturali che motivano questa scelta sono:

\begin{enumerate}
    \item \textbf{Design anchor-free}: YOLOv8 abbandona completamente il paradigma delle anchor box a favore di una formulazione \emph{anchor-free}, eliminando la necessità di definire a priori le proporzioni dei bounding box e riducendo significativamente il numero di iperparametri.
    
    \item \textbf{Backbone CSPDarknet migliorato}: il backbone incorpora connessioni \emph{Cross-Stage Partial} (CSP) che permettono un flusso del gradiente più efficiente rispetto al Darknet-53 originale di YOLOv3, migliorando sia la convergenza sia la qualità delle feature estratte.
    
    \item \textbf{Neck C2f}: il modulo \emph{Cross-Stage Partial with 2 convolutions + flow} (C2f) sostituisce il tradizionale PANet, consentendo un'aggregazione multi-scala delle feature più ricca e computazionalmente efficiente.
    
    \item \textbf{Decoupled Head}: la testa di predizione separa esplicitamente il ramo di classificazione da quello di regressione delle bounding box, permettendo a ciascun ramo di specializzarsi indipendentemente.
    
    \item \textbf{Loss function avanzata}: YOLOv8 utilizza la \emph{Distribution Focal Loss} (DFL) per la regressione e la \emph{Binary Cross-Entropy} per la classificazione, con un meccanismo di \emph{Task-Aligned Assignment} che migliora l'assegnazione dei target ai predittori durante il training.
\end{enumerate}
...
Lo script \texttt{task2\_train\_yolo.py} sfrutta l'API di alto livello della libreria \texttt{ultralytics} per configurare e lanciare il training:

\begin{lstlisting}[language=Python, basicstyle=\small\ttfamily, frame=single, caption={Configurazione del training YOLOv8.}]
model = YOLO(weights)  # Carica pesi pre-addestrati (COCO)
results = model.train(
    data=data_yaml,    # Percorso del dataset.yaml
    epochs=epochs,     # Numero massimo di epoche
    batch=batch,       # Batch size
    device=0,          # Prima GPU disponibile
    plots=True,        # Genera grafici automatici
    save=True,         # Salva best.pt
    exist_ok=True      # Sovrascrive run precedenti
)
\end{lstlisting}
...
\subsection{Curva Precision-Recall e F1-Confidence}

Le Figure~\ref{fig:pr_curve} e~\ref{fig:f1_curve} mostrano rispettivamente la curva Precision-Recall e la curva F1-Confidence del modello YOLOv8x.

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{task2_poi_xlarge/BoxPR_curve.png}
        \caption{Curva Precision-Recall. La curva blu spessa rappresenta la media su tutte le classi (mAP@50 = 0.297). Le curve grigie sottili mostrano le singole classi, evidenziando la forte eterogeneità delle performance.}
        \label{fig:pr_curve}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{task2_poi_xlarge/BoxF1_curve.png}
        \caption{Curva F1-Confidence. Il massimo F1 medio (0.36) viene raggiunto con una soglia di confidenza di 0.759, indicando che il modello è altamente selettivo nelle sue predizioni.}
        \label{fig:f1_curve}
    \end{subfigure}
    \caption{Curve di valutazione del modello YOLOv8x.}
    \label{fig:curves}
\end{figure}
