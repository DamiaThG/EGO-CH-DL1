...
def find_local_label_file(seq_dir):
    ...
    if len(candidates) == 1:
        ...
        dir_num = re.search('\\d+', seq_dir.name)
        if dir_num:
            num_str = dir_num.group()
            for c in candidates:
                c_num = re.search('\\d+', c.name)
...
def main():
    ...
    for seq_dir, jpg_files in tqdm(sequences.items(), desc='Processing Sequences'):
        ...
        for i, img_path in enumerate(jpg_files):
            ...
            if train_mapping or val_mapping:
                if isinstance(train_mapping, list) and len(train_mapping) > 0:
                    ...
                    target_split = 'val'
                elif isinstance(train_mapping, dict) and key in train_mapping:
            ...
            if target_split == 'train':
                ...
                train_labels.append(room_id)
                train_ids.append(frame_idx)
            else:
                val_features.append(feat)
                val_labels.append(room_id)
                val_ids.append(frame_idx)
        if len(train_features) > 0:
            features_tensor = torch.stack(train_features)
            del train_features
            data = {'video_id': video_id, 'features': features_tensor, 'room_labels': torch.tensor(train_labels, dtype=torch.long), 'frame_ids': torch.tensor(train_ids, dtype=torch.long)}
        ...
        if len(val_features) > 0 and args.val_output_dir:
            features_tensor = torch.stack(val_features)
            del val_features
            data = {'video_id': video_id, 'features': features_tensor, 'room_labels': torch.tensor(val_labels, dtype=torch.long), 'frame_ids': torch.tensor(val_ids, dtype=torch.long)}
