...
from Scripts.Task1_Localization.v3_experimental.task1_dataset import get_dataloaders
...
class Task1LightningModule(L.LightningModule):
    ...
    def __init__(self, model, num_classes, class_weights=None, lr=0.001, weight_decay=0.0001, max_epochs=50, label_smoothing=0.0):
        super().__init__()
        self.model = model
        ...
        self.loss_fn = torch.nn.CrossEntropyLoss(weight=self.class_weights, ignore_index=-100, label_smoothing=label_smoothing)
        self._val_preds = []
        self._val_labels = []
...
def main():
    ...
    from Scripts.Task1_Localization.models.task1_model_mamba_v3 import MambaRoomLocalizerV3
    model = MambaRoomLocalizerV3(input_dim=384, d_model=args.d_model, num_layers=args.num_layers, num_classes=num_classes, d_state=args.d_state, dropout=args.dropout)
    class_weights = train_loader.dataset.class_weights if hasattr(train_loader.dataset, 'class_weights') else None
    lit_model = Task1LightningModule(model=model, num_classes=num_classes, class_weights=class_weights, lr=args.lr, weight_decay=args.weight_decay, max_epochs=args.epochs, label_smoothing=args.label_smoothing)
